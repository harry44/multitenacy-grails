 ...
 */
public class HACsvConverter extends CsvConverter{

    private static final Log log = LogFactory.getLog(HACsvConverter.class);
     ...
     * @param user
     */
    protected HACsvConverter(MROldFonteCommissione fonteCommissione, MROldPagamento pagamento, User user) {
    ...
    public void convert(
        ...
        List<HACsvReservation> reservations = new ArrayList<HACsvReservation>();
        ...
        while (line != null) {
            HACsvRecord csvRecord = null;
            try {
                csvRecord = new HACsvRecord(line);
            } catch (Exception ex) {
                csvRecord = null;
                log.debug("Parse error for line: " + line);
            }
            if (csvRecord != null) {
                HACsvReservation csvReservation = new HACsvReservation(csvRecord);
    ...
    public String[] saveCSVResponse(Session sx, String originalFilename) {
        ...
        if (originalFilename != null) {
            String baseName = originalFilename.replaceAll("[.][^.]*$", ""). // Cut extension.
                    replaceAll("holidayautos_[0-9]+_(confirmed|not_confirmed|errors)_?", ""); // Delete old prefix
        ...
        StringBuffer prefix = new StringBuffer().append("holidayautos").append("_").append(cookie()).append("_");

        String fileNameConfirmed = new StringBuffer().append(prefix).append("confirmed").append(suffix).toString();
        String fileNameNotConfirmed = new StringBuffer().append(prefix).append("not_confirmed").append(suffix).toString();
        String fileNameErrors = new StringBuffer().append(prefix).append("errors").append(suffix).toString();
        ...
        try {
            ...
            fileNameErrors = null;
            log.debug("Could not save CSV response", ex);
        } finally {
